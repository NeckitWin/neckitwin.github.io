...
const CustomButton = ({links}:CustomButtonProps): React.ReactNode => {
    return (
        <ul className='flex flex-row gap-2 md:gap-6'>
            {links.map((link) => (
            <li key={link.name} className=' transition ease-linear hover:-translate-y-0.5'>
                <a href={link.link} rel='noreferrer' target='_blank'
                   className='text-white px-2 py-1.5 rounded-lg shadow-md bg-neutral-900/50 shadow-neutral-900/90 text-xs sm:text-base whitespace-nowrap'>
                    {link.name}
                </a>
            </li>
            ))}
